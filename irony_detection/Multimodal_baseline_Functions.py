...
Training_path = "E:\\Master\\Disertatie\\DISERATATIE\\Training_memes_dateset_test.csv"
Validation_path = "E:\\Master\\Disertatie\\DISERATATIE\\Validation_memes_dataset.csv"
Testing_path = "E:\\Master\\Disertatie\\DISERATATIE\\Testing_memes_dataset.csv"
img_dir = "E:\\Master\\Disertatie\\DISERATATIE\\PoliticalMemes"
...
def preprocess_text(Training_path, Validation_path, Testing_path):
    ...
    try:
        training_DF = pd.read_csv(Training_path, sep=',', encoding='latin-1')
    except UnicodeDecodeError:
        # În cazul în care apare o eroare la decodificare, afișăm un mesaj și continuăm
        print("Eroare de decodificare UTF-8. Încercăm să citim cu alt encoding.")
        training_DF = pd.read_csv(Training_path, sep=',', encoding='ISO-8859-1')
    #training_DF = pd.read_csv(Training_path, sep=',', encoding='latin-1', errors='ignore')
    #training_DF = pd.read_csv(Training_path, sep=',')
    validation_DF = pd.read_csv(Validation_path, sep=',', encoding='latin-1')
    testing_DF = pd.read_csv(Testing_path, sep=',', encoding='latin-1')
...
def text_generator2(padded_seq, y, batch_size=None):
    """
        padded_seq: vectorized padded text sequence
        y: label of the text
        batch_size: Number of observations to be selected at a time
...
def img_text_generator(files, padded_seq, y, batch_size=None):
    ...
    """
    while True:
        batch_idxs = np.random.choice(a=list(range(len(padded_seq))),
                                      size=batch_size)  # Selecting the random batch indexes
        batch_input_txt = []  # Initializing batch input text
        batch_input_img = []  # Initializing batch input image
        batch_output = []  # Initializing batch output
